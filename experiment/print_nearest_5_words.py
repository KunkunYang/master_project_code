...
def find_nearest_n_word(F_i, name, numofword):

    # read embedding
    emb_word= []
    emb_array= []

    emb_size = np.size(F_i)
    with open(name, "r", encoding = 'latin-1') as f:
        for line in f:
            sp = line.split(' ')
            emb_word.append(sp[0])
            emb_array.append([float(i) for i in sp[1:emb_size+1]])

    emb_array = np.array(emb_array)

    a = np.dot(emb_array, F_i)
            
    list_num = a.argsort()[-numofword:][::-1]
    list_words = []
    for i in range(numofword):
        list_words.append(emb_word[list_num[i]])
...
print('########## Enter number of clusters for each algorithm: ')
num_labels_semi = int(input("num semi: "))
num_labels_conv =int(input("num conv: "))
num_labels_fuzz =int(input("num fuzz 2: "))
num_labels_fuzz11 =int(input("num fuzz 1.1: "))

vocab_name =  'vocabulary/'+ file + '_emb_' + embedding + '.txt'
...
print('########## Nearest 5 words printed in LaTeX table format')
...
for i in range(numofrounds):
    ...
    MM = fuzz.cluster.cmeans(np.transpose(data_matrix), num_labels_fuzz, 2, error = tol, maxiter=numofiter, init = None)
    ctr_2 = np.transpose(MM[0])
    G_fuzz = np.transpose(MM[1])
    res_fuzz = (MM[4])[-1]

    MM_11 = fuzz.cluster.cmeans(np.transpose(data_matrix), num_labels_fuzz, 1.1, error = tol, maxiter=numofiter, init = None)
    ctr_11 = np.transpose(MM_11[0])
    G_fuzz = np.transpose(MM_11[1])
    res_fuzz = (MM_11[4])[-1]
    ...
    F, G_semi, res_semi = semi_NMF(data_matrix, num_labels_semi, numofiter, tol)
    W, G_conv, res_conv = convex_NMF(data_matrix, num_labels_conv, numofiter, tol)

    #Semi
    numofword = 5
        
    print('Semi: ')
    for j in range(num_labels_semi):
        list_temp = find_nearest_n_word(F[:,j], vocab_name, numofword)
        print(j + 1, ' & ', end='')
        for x in range(numofword):
            print(list_temp[x], end='')
    ...
    F = np.dot(np.transpose(data_matrix), W)

    for j in range(num_labels_conv):
        list_temp = find_nearest_n_word(F[:,j], vocab_name, numofword)
        print(j + 1, ' & ', end='')
        for x in range(numofword):
            print(list_temp[x], end='')
    ...
    for j in range(num_labels_fuzz):
        list_temp = find_nearest_n_word(ctr_2[:,j], vocab_name, numofword)
        print(j + 1, ' & ', end='')
        for x in range(numofword):
            print(list_temp[x], end='')
    ...
    for j in range(num_labels_fuzz):
        list_temp = find_nearest_n_word(ctr_11[:,j], vocab_name, numofword)
        print(j + 1, ' & ', end='')
        for x in range(numofword):
            print(list_temp[x], end='')
